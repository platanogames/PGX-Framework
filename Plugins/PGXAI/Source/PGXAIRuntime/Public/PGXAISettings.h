...
#include "PGXAISettings.generated.h"

class UPGXAIConfig;

/**
 * Reserved project settings; PGX AI runtime does not currently resolve configuration through them.
 * These values are editable but are not consumed by the runtime subsystem yet.
 */
UENUM(BlueprintType)
enum class EPGXAIDiscoveryMode : uint8
{
	/** EN: Scan AssetRegistry for UPGXAIConfig DAs (deprecated; only consulted when ActiveConfig is unset). */
...
UCLASS(config = Game, defaultconfig, meta = (DisplayName = "PGX AI System"))
class PGXAIRUNTIME_API UPGXAISettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TSoftObjectPtr<UPGXAIConfig> ActiveConfig;
	...
	EPGXAIDiscoveryMode DiscoveryMode = EPGXAIDiscoveryMode::AssetRegistryScan;
