...
void UPGXAudioSubsystem::RegisterConsoleCommands()
{
	// EN: Register 20 console commands for the audio system
	// ES: Registrar 20 comandos de consola para el sistema de audio

	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.status"),
		TEXT("Show PGX Audio system status"),
	...
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.channels"),
		TEXT("Show channel volumes and mute states"),
		FConsoleCommandDelegate::CreateWeakLambda(this, [this]()
		{
			const TArray<FPGXAudioChannelSnapshot> Channels = GetAllChannelStates();
	...
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.playing"),
		TEXT("Show active sound list"),
		FConsoleCommandDelegate::CreateWeakLambda(this, [this]()
		{
	...
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.music"),
		TEXT("Show music manager state"),
		FConsoleCommandDelegate::CreateWeakLambda(this, [this]()
		{
	...
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.pool"),
		TEXT("Show sound pool statistics"),
		FConsoleCommandDelegate::CreateWeakLambda(this, [this]()
		{
	...
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.memory"),
		TEXT("Show audio memory usage"),
		FConsoleCommandDelegate::CreateWeakLambda(this, [this]()
		{
	...
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.history"),
		TEXT("Show last 20 audio events"),
		FConsoleCommandDelegate::CreateWeakLambda(this, [this]()
		{
	...
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.backend"),
		TEXT("Show backend details"),
		FConsoleCommandDelegate::CreateWeakLambda(this, [this]()
		{
	...
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.dialogue"),
		TEXT("Show dialogue queue"),
		FConsoleCommandDelegate::CreateWeakLambda(this, [this]()
		{
	...
	// EN: pgx.audio.set <channel_tag> <volume> — Set channel volume
	// ES: pgx.audio.set <channel_tag> <volumen> — Establecer volumen de canal
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.set"),
		TEXT("Set channel volume: pgx.audio.set PGX.Audio.Channel.Music 0.5"),
	...
	// EN: pgx.audio.mute <channel_tag|all> — Toggle mute
	// ES: pgx.audio.mute <channel_tag|all> — Alternar mute
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.mute"),
		TEXT("Toggle mute: pgx.audio.mute PGX.Audio.Channel.Music | pgx.audio.mute all"),
		FConsoleCommandWithArgsDelegate::CreateWeakLambda(this, [this](const TArray<FString>& Args)
		{
			if (!AreConsoleMutationsAllowed())
			{
	...
	// EN: pgx.audio.play <sound_tag> — Test-play a resolved sound
	// ES: pgx.audio.play <sound_tag> — Reproducir un sonido resuelto de prueba
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.play"),
		TEXT("Test-play a resolved sound by SoundDefinition tag"),
		FConsoleCommandWithArgsDelegate::CreateWeakLambda(this, [this](const TArray<FString>& Args)
		{
			if (!AreConsoleMutationsAllowed())
			{
	...
	// EN: pgx.audio.switch <Legacy|Modulation> — Switch backend
	// ES: pgx.audio.switch <Legacy|Modulation> — Cambiar backend
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.switch"),
		TEXT("Switch audio backend: pgx.audio.switch Legacy|Modulation"),
		FConsoleCommandWithArgsDelegate::CreateWeakLambda(this, [this](const TArray<FString>& Args)
		{
			if (!AreConsoleMutationsAllowed())
			{
	...
	// EN: pgx.audio.debug [on|off] — Toggle debug overlay
	// ES: pgx.audio.debug [on|off] — Alternar overlay de debug
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.debug"),
		TEXT("Toggle debug overlay: pgx.audio.debug on|off"),
		FConsoleCommandWithArgsDelegate::CreateWeakLambda(this, [this](const TArray<FString>& Args)
		{
			if (!AreConsoleMutationsAllowed())
			{
	...
	// EN: pgx.audio.mix — Show 5-layer mix state
	// ES: pgx.audio.mix — Mostrar estado de mezcla de 5 capas
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.mix"),
		TEXT("Show 5-layer mix state"),
		FConsoleCommandDelegate::CreateWeakLambda(this, [this]()
		{
	...
	// EN: pgx.audio.ducking — Show active ducking rules
	// ES: pgx.audio.ducking — Mostrar reglas de ducking activas
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.ducking"),
		TEXT("Show active ducking rules"),
		FConsoleCommandDelegate::CreateWeakLambda(this, [this]()
		{
			UPGXAudioMixSubsystem* MixSub = FindMixSubsystem();
			if (!MixSub)
	...
	// EN: pgx.audio.hdr [on|off] — Toggle HDR audio
	// ES: pgx.audio.hdr [on|off] — Alternar audio HDR
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.hdr"),
		TEXT("Toggle HDR audio: pgx.audio.hdr on|off"),
		FConsoleCommandWithArgsDelegate::CreateWeakLambda(this, [this](const TArray<FString>& Args)
		{
			if (!AreConsoleMutationsAllowed())
			{
	...
	// EN: pgx.audio.hrtf [on|off] — Toggle HRTF
	// ES: pgx.audio.hrtf [on|off] — Alternar HRTF
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.hrtf"),
		TEXT("Toggle HRTF: pgx.audio.hrtf on|off"),
		FConsoleCommandWithArgsDelegate::CreateWeakLambda(this, [this](const TArray<FString>& Args)
		{
			if (!AreConsoleMutationsAllowed())
			{
	...
	// EN: pgx.audio.device [name] — Query or set audio device
	// ES: pgx.audio.device [name] — Consultar o establecer dispositivo de audio
	RegisteredCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("pgx.audio.device"),
		TEXT("Query/set audio device: pgx.audio.device [device_name]"),
		FConsoleCommandWithArgsDelegate::CreateWeakLambda(this, [this](const TArray<FString>& Args)
		{
			UPGXAudioMixSubsystem* MixSub = FindMixSubsystem();
			if (!MixSub)
	...
	PGX_LOG_INFO(LogPGXAudio, TEXT("RegisterConsoleCommands — %d commands registered"), RegisteredCommands.Num());
}

void UPGXAudioSubsystem::UnregisterConsoleCommands()
{
