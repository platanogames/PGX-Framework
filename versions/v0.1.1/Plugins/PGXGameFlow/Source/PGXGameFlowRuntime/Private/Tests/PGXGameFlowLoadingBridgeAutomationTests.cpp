...
namespace PGXGameFlowLoadingBridgeAutomation
{
	struct FScopedGameInstanceFixture
	{
			...
			: Test(InTest)
		{
			if (!GEngine)
			{
				Test.AddError(TEXT("PGXGameFlow loading bridge automation setup failed: engine is unavailable."));
				return;
			}
			...
			GameInstance->InitializeStandalone(TEXT("PGXGameFlowLoadingBridgeAutomationWorld"));
		}
