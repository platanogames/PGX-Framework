...
namespace PGXGameFlowAutomation
{
	...
	{
		explicit FScopedGameInstanceFixture(
			FAutomationTestBase& InTest,
			const TCHAR* WorldName = TEXT("PGXGameFlowAutomationWorld"))
			: Test(InTest)
		{
			if (!GEngine)
			{
				Test.AddError(TEXT("PGXGameFlow automation setup failed: engine is unavailable."));
				return;
			}
...
PGX_TEST_GAME(FPGXGameFlow_ConsoleOwnershipWorldRoutingAutomationTest)
{
	auto ConfigureForConsoleMutation = [this](UGameInstance* GameInstance, const TCHAR* ConfigName)
	{
		UPGXGameFlowSubsystem* GameFlow = PGXGameFlowAutomation::FindGameFlow(*this, GameInstance);
		if (!GameFlow)
		{
			return static_cast<UPGXGameFlowSubsystem*>(nullptr);
		}

		UPGXGameFlowConfig* Config = PGXGameFlowAutomation::MakeConfig(ConfigName);
		Config->bAllowConsoleMutations = true;
		GameFlow->InjectTestConfig(Config);
		GameFlow->InjectTestRulesConfigs({});
		GameFlow->ResetChannelStatesForTesting();
		return GameFlow;
	};

	auto ExecuteSet = [this](UWorld* World, const FGameplayTag& Tag)
	{
		const FString Input = FString::Printf(TEXT("pgx.gameflow.set 1 %s"), *Tag.ToString());
		const bool bHandled = IConsoleManager::Get().ProcessUserConsoleInput(*Input, *GLog, World);
		TestTrue(TEXT("World-aware console command was handled"), bHandled);
	};

	{
		PGXGameFlowAutomation::FScopedGameInstanceFixture First(*this, TEXT("PGXGameFlowConsoleWorld_FirstA"));
		PGXGameFlowAutomation::FScopedGameInstanceFixture Second(*this, TEXT("PGXGameFlowConsoleWorld_SecondA"));
		UPGXGameFlowSubsystem* FirstFlow = ConfigureForConsoleMutation(First.Get(), TEXT("PGXGameFlowConsoleConfig_FirstA"));
		UPGXGameFlowSubsystem* SecondFlow = ConfigureForConsoleMutation(Second.Get(), TEXT("PGXGameFlowConsoleConfig_SecondA"));
		if (FirstFlow && SecondFlow)
		{
			ExecuteSet(First.Get()->GetWorld(), TAG_PGX_GameFlow_State_MainMenu);
			TestTrue(TEXT("Command routes to the supplied first world"),
				FirstFlow->GetCurrentFlowTag(EPGXFlowChannel::UI) == TAG_PGX_GameFlow_State_MainMenu);
			TestFalse(TEXT("Command does not mutate the overlapping second world"),
				SecondFlow->GetCurrentFlowTag(EPGXFlowChannel::UI).IsValid());

			First.Shutdown();
			ExecuteSet(Second.Get()->GetWorld(), TAG_PGX_GameFlow_State_Loading);
			TestTrue(TEXT("Second world remains routable after first-world teardown"),
				SecondFlow->GetCurrentFlowTag(EPGXFlowChannel::UI) == TAG_PGX_GameFlow_State_Loading);
		}
	}

	{
		PGXGameFlowAutomation::FScopedGameInstanceFixture First(*this, TEXT("PGXGameFlowConsoleWorld_FirstB"));
		PGXGameFlowAutomation::FScopedGameInstanceFixture Second(*this, TEXT("PGXGameFlowConsoleWorld_SecondB"));
		UPGXGameFlowSubsystem* FirstFlow = ConfigureForConsoleMutation(First.Get(), TEXT("PGXGameFlowConsoleConfig_FirstB"));
		UPGXGameFlowSubsystem* SecondFlow = ConfigureForConsoleMutation(Second.Get(), TEXT("PGXGameFlowConsoleConfig_SecondB"));
		if (FirstFlow && SecondFlow)
		{
			Second.Shutdown();
			ExecuteSet(First.Get()->GetWorld(), TAG_PGX_GameFlow_State_InWorld);
			TestTrue(TEXT("First world remains routable after second-world teardown"),
				FirstFlow->GetCurrentFlowTag(EPGXFlowChannel::UI) == TAG_PGX_GameFlow_State_InWorld);
		}
	}

	IConsoleManager& ConsoleManager = IConsoleManager::Get();
	IConsoleCommand* Sentinel = ConsoleManager.RegisterConsoleCommand(
		TEXT("pgx.w2.external.sentinel"),
		TEXT("External sentinel for W2 module-lifetime ownership validation"),
		FConsoleCommandDelegate::CreateLambda([]() {}),
		ECVF_Default);
	TestNotNull(TEXT("External sentinel registered"), Sentinel);

	FPGXGameFlowRuntimeModule* Module =
		FModuleManager::GetModulePtr<FPGXGameFlowRuntimeModule>(TEXT("PGXGameFlowRuntime"));
	TestNotNull(TEXT("GameFlow runtime module is loaded"), Module);
	if (Module)
	{
		TestNotNull(TEXT("Owned command exists before module restart"),
			ConsoleManager.FindConsoleObject(TEXT("pgx.gameflow.status")));
		Module->ShutdownModule();
		TestNull(TEXT("Owned command is removed on module shutdown"),
			ConsoleManager.FindConsoleObject(TEXT("pgx.gameflow.status")));
		TestTrue(TEXT("Unrelated sentinel survives module shutdown"),
			ConsoleManager.FindConsoleObject(TEXT("pgx.w2.external.sentinel")) == Sentinel);

		Module->StartupModule();
		IConsoleObject* RestartedCommand = ConsoleManager.FindConsoleObject(TEXT("pgx.gameflow.status"));
		TestNotNull(TEXT("Owned command returns on module startup"), RestartedCommand);
		Module->StartupModule();
		TestTrue(TEXT("Repeated startup preserves the same command registration"),
			ConsoleManager.FindConsoleObject(TEXT("pgx.gameflow.status")) == RestartedCommand);
	}

	if (Sentinel)
	{
		ConsoleManager.UnregisterConsoleObject(Sentinel);
	}
	TestNull(TEXT("External sentinel cleanup completed"),
		ConsoleManager.FindConsoleObject(TEXT("pgx.w2.external.sentinel")));
	return true;
}
...
PGX_TEST_GAME(FPGXGameFlow_DuplicateRulesConflictPolicyAutomationTest)
{
	...
	UPGXFlowRulesConfig* RulesUI = PGXGameFlowAutomation::MakeRules(TEXT("PGXGameFlow_AutomationRules_UI"), EPGXFlowChannel::UI, 4);
