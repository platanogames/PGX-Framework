...
void UPGXGameFlowSubsystem::BuildRulesCache()
{
	...
	Algo::Sort(DiscoveredRulesConfigs, [](const TObjectPtr<UPGXFlowRulesConfig>& A, const TObjectPtr<UPGXFlowRulesConfig>& B)
	{
		if (!A) { return false; }
		if (!B) { return true; }
...
void UPGXGameFlowSubsystem::ExecuteConsoleCommand(const FString& CommandName, const TArray<FString>& Args, UWorld* World)
{
	if (CommandName == TEXT("pgx.gameflow.canchange"))
	{
		if (Args.Num() < 2)
		{
	...
	if (CommandName == TEXT("pgx.gameflow.history"))
	{
		if (Args.Num() < 1)
		{
	...
	if (CommandName == TEXT("pgx.gameflow.revert"))
	{
	...
	if (CommandName == TEXT("pgx.gameflow.rules"))
	{
		if (Args.Num() < 1)
		{
	...
	if (CommandName == TEXT("pgx.gameflow.set"))
	{
		if (!IsConsoleMutationAllowed())
		{
	...
	if (CommandName == TEXT("pgx.gameflow.status"))
	{
