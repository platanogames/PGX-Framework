...
void UPGXPSOSubsystem::ExecuteConsoleCommand(const FString& CommandName, const TArray<FString>& Args, UWorld* World)
{
	if (CommandName == TEXT("pgx.pso.cancel"))
	{
		CancelWarmUp();
		PGX_LOG_INFO(LogPGXPSO, TEXT("pgx.pso.cancel: Warm-up cancelled"));
		return;
	}
	if (CommandName == TEXT("pgx.pso.configs"))
	{
	...
	if (CommandName == TEXT("pgx.pso.record.clear"))
	{
		#if WITH_EDITOR
					ClearRecording();
		#else
	...
	if (CommandName == TEXT("pgx.pso.record.export"))
	{
		#if WITH_EDITOR
	...
	if (CommandName == TEXT("pgx.pso.record.start"))
	{
		#if WITH_EDITOR
	...
	if (CommandName == TEXT("pgx.pso.record.stop"))
	{
		#if WITH_EDITOR
					StopRecording();
		#else
	...
	if (CommandName == TEXT("pgx.pso.status"))
	{
	...
	if (CommandName == TEXT("pgx.pso.validate"))
	{
	...
	if (CommandName == TEXT("pgx.pso.warmup"))
	{
		if (Args.Num() == 0)
		{
