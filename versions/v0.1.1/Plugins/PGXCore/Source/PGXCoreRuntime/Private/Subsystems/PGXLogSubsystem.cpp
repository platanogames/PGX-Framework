...
void UPGXLogSubsystem::ExecuteConsoleCommand(const FString& CommandName, const TArray<FString>& Args, UWorld* World)
{
	if (CommandName == TEXT("pgx.log.clear"))
	{
		RingBuffer.Clear();
		UE_LOG(LogPGXLog, Log, TEXT("Ring buffer cleared"));
		return;
	}
	if (CommandName == TEXT("pgx.log.domains"))
	{
	...
	if (CommandName == TEXT("pgx.log.export"))
	{
	...
	if (CommandName == TEXT("pgx.log.filter"))
	{
		if (Args.Num() == 0)
		{
	...
	if (CommandName == TEXT("pgx.log.list"))
	{
	...
	if (CommandName == TEXT("pgx.log.stats"))
	{
