...
void UPGXEventHandlerSubsystem::ExecuteConsoleCommand(const FString& CommandName, const TArray<FString>& Args, UWorld* World)
{
	if (CommandName == TEXT("pgx.event.blackbox"))
	{
		UE_LOG(LogPGXEventHandler, Log, TEXT("%s"), *DumpBlackboxToString());
		return;
	}
	if (CommandName == TEXT("pgx.event.cache"))
	{
	...
	if (CommandName == TEXT("pgx.event.cache.clear"))
	{
		InvalidateCache();
		UE_LOG(LogPGXEventHandler, Log, TEXT("Handler cache cleared."));
		return;
	}
	if (CommandName == TEXT("pgx.event.list"))
	{
	...
	if (CommandName == TEXT("pgx.event.report"))
	{
		UE_LOG(LogPGXEventHandler, Log, TEXT("%s"), *ExportReport());
		return;
	}
	if (CommandName == TEXT("pgx.event.status"))
	{
	...
	if (CommandName == TEXT("pgx.event.telemetry"))
	{
