...
#include "PGXCoreHUD.generated.h"
...
/**
 * EN: Base HUD for PGX projects (L1 Core). Supports construction DA resolution
 *     for widget layers and component injection.
 *     Blueprint users can configure per-instance bridge wiring via UPROPERTY.
 *     Named PGXCoreHUD to avoid UHT conflict with PGXUIRuntime's PGXHUDBase.
 * ES: HUD base para proyectos PGX (L1 Core). Soporta resolucion de DA de construccion
 *     para capas de widgets e inyeccion de componentes.
 *     Usuarios de Blueprint pueden configurar bridge wiring per-instancia via UPROPERTY.
 *     Nombrado PGXCoreHUD para evitar conflicto UHT con PGXHUDBase de PGXUIRuntime.
 */
UCLASS(BlueprintType, Blueprintable)
class PGXCORERUNTIME_API APGXCoreHUD : public AHUD
{
	GENERATED_BODY()
...
public:
	...
	TSoftObjectPtr<UPGXHUDConstruction> ConstructionOverride;
...
protected:
	...
	void ApplyConstruction(const UPGXHUDConstruction* Construction);
	...
	/** EN: Called before a save operation for this HUD's domain / ES: Llamado antes de save */
	UFUNCTION(BlueprintNativeEvent, Category = "PGX|Save")
	void OnPGXPreSave(const FPGXBridgeSaveNotification& Notification);

	/** EN: Called after a load operation for this HUD's domain / ES: Llamado despues de load */
	UFUNCTION(BlueprintNativeEvent, Category = "PGX|Save")
