...
#include "PGXAsyncAction_Load.h"
#include "PGXSaveSubsystem.h"
#include "PGXSaveConfig.h"
#include "Engine/GameInstance.h"

// EN: Async latent action for loading with auto-resolved slot name
// ES: Accion latente async para cargar con slot auto-resuelto

UPGXAsyncAction_Load* UPGXAsyncAction_Load::LoadAsync(
	UObject* WorldContextObject,
	FGameplayTag ContextTag)
{
	UPGXAsyncAction_Load* Action = NewObject<UPGXAsyncAction_Load>();
	Action->RequestedContextTag = ContextTag;
	Action->RegisterWithGameInstance(WorldContextObject);
	return Action;
}

void UPGXAsyncAction_Load::Activate()
{
	...
	TWeakObjectPtr<UPGXAsyncAction_Load> WeakThis(this);
	...
	CompletedHandle = SaveSub->OnLoadCompletedNative.AddLambda(
		[WeakThis, CapturedSlot, SaveSub](const FString& SlotName, EPGXSaveResult Result, UPGXSaveGame* /*SaveGame*/)
		{
			...
			if (UPGXAsyncAction_Load* Self = WeakThis.Get())
			{
	...
	// EN: Initiate the async load with auto-resolved slot
	// ES: Iniciar la carga async con slot auto-resuelto
	SaveSub->LoadContextAsync(RequestedContextTag, ResolvedSlotName);
}

void UPGXAsyncAction_Load::BeginDestroy()
{
