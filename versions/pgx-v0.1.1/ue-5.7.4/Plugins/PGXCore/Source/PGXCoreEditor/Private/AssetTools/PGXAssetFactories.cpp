...
namespace
{
	...
	{
		// EN: Factory CDOs are built while PGXCoreEditor is loading, before StartupModule.
		//     Load the class's /Script provider first so class resolution is not an
		//     order-dependent probe that emits a false missing-object warning.
		// ES: Los CDO de fabrica se construyen mientras carga PGXCoreEditor, antes de
		//     StartupModule. Cargar primero el proveedor /Script evita una consulta
		//     dependiente del orden y su warning falso de objeto ausente.
		const FString ClassPathString(ClassPath);
		constexpr int32 ScriptPrefixLength = 8; // "/Script/"
		int32 ClassSeparatorIndex = INDEX_NONE;
		if (ClassPathString.StartsWith(TEXT("/Script/"))
			&& ClassPathString.FindChar(TEXT('.'), ClassSeparatorIndex))
		{
			const FName ProviderModuleName(*ClassPathString.Mid(
				ScriptPrefixLength,
				ClassSeparatorIndex - ScriptPrefixLength));
			if (!FModuleManager::Get().IsModuleLoaded(ProviderModuleName)
				&& !FModuleManager::Get().LoadModule(ProviderModuleName))
			{
				UE_LOG(LogPGXCoreEditor, Warning, TEXT("PGX asset factory could not load provider module: %s"), *ProviderModuleName.ToString());
				return nullptr;
			}
		}

		UClass* ResolvedClass = StaticLoadClass(
		...
		if (!ResolvedClass)
		{
			UE_LOG(LogPGXCoreEditor, Warning, TEXT("PGX asset factory could not load class: %s"), ClassPath);
			return nullptr;
		}
