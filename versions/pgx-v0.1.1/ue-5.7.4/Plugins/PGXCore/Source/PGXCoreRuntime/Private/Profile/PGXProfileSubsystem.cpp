...
void UPGXProfileSubsystem::ExecuteConsoleCommand(const FString& CommandName, const TArray<FString>& Args, UWorld* World)
{
	if (CommandName == TEXT("pgx.profile.budget"))
	{
		if (Args.Num() < 1)
		{
	...
	if (CommandName == TEXT("pgx.profile.capability"))
	{
		if (Args.Num() < 1)
		{
	...
	if (CommandName == TEXT("pgx.profile.dump"))
	{
	...
	if (CommandName == TEXT("pgx.profile.feature"))
	{
		if (Args.Num() < 1)
		{
...
#if WITH_EDITOR
	if (CommandName == TEXT("pgx.profile.simulate.build"))
	{
		if (Args.Num() < 1)
		{
...
#endif
#if WITH_EDITOR
	if (CommandName == TEXT("pgx.profile.simulate.clear"))
	{
...
#endif
#if WITH_EDITOR
	if (CommandName == TEXT("pgx.profile.simulate.platform"))
	{
		if (Args.Num() < 1)
		{
...
#endif
#if WITH_EDITOR
	if (CommandName == TEXT("pgx.profile.simulate.status"))
	{
		UE_LOG(LogPGX, Log, TEXT(""));
...
#endif
	if (CommandName == TEXT("pgx.profile.status"))
	{
		UE_LOG(LogPGX, Log, TEXT(""));
