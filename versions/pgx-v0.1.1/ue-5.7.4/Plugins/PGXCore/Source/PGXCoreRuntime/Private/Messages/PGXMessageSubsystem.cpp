...
void UPGXMessageSubsystem::ExecuteConsoleCommand(const FString& CommandName, const TArray<FString>& Args, UWorld* World)
{
	if (CommandName == TEXT("pgx.message.broadcast"))
	{
	...
	if (CommandName == TEXT("pgx.message.channels"))
	{
	...
	if (CommandName == TEXT("pgx.message.history"))
	{
	...
	if (CommandName == TEXT("pgx.message.status"))
	{
