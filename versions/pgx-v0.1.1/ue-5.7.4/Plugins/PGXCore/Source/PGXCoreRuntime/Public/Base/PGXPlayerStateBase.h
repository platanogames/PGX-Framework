...
#include "PGXPlayerStateBase.generated.h"
...
/**
 * EN: Base player state for PGX projects. Supports construction DA resolution
 *     for initial player tags and component injection.
 *     Blueprint users can configure per-instance bridge wiring via UPROPERTY.
 * ES: Player state base para proyectos PGX. Soporta resolucion de DA de construccion
 *     para tags iniciales del jugador e inyeccion de componentes.
 *     Usuarios de Blueprint pueden configurar bridge wiring per-instancia via UPROPERTY.
 */
UCLASS(BlueprintType, Blueprintable)
class PGXCORERUNTIME_API APGXPlayerStateBase : public APlayerState
{
	GENERATED_BODY()
...
public:
	...
	TSoftObjectPtr<UPGXPlayerStateConstruction> ConstructionOverride;
...
protected:
	...
	void ApplyConstruction(const UPGXPlayerStateConstruction* Construction);
