...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FPGXInput_SettingsConfigLifecycleAutomationTest,
	"PGX.Input.SettingsConfigLifecycle", PGX_INPUT_AUTOMATION_FLAGS)
bool FPGXInput_SettingsConfigLifecycleAutomationTest::RunTest(const FString& Parameters)
{
	UPGXInputSettings* Settings = GetMutableDefault<UPGXInputSettings>();
	if (!TestNotNull(TEXT("SettingsConfigLifecycle mutable settings"), Settings))
	{
		return false;
	}

	const TSoftObjectPtr<UPGXInputConfig> OriginalConfig = Settings->ActiveConfig;
	UPGXInputConfig* Config = PGXInputAutomation::MakeConfig(TEXT("PGXInput_Automation_SettingsConfig"), 7, 0.35f);
	UPGXInputContext* Context = PGXInputAutomation::MakeContext(
		TEXT("PGXInput_Automation_SettingsContext"), TAG_PGX_Input_Context_Default.GetTag(), 6);
	FPGXInputContextEntry Entry;
	Entry.ContextTag = Context->ContextTag;
	Entry.Context = Context;
	Config->DefaultContexts.Add(Entry);
	...
	for (int32 Cycle = 0; Cycle < 2; ++Cycle)
	{
		PGXInputAutomation::FScopedInputSubsystemFixture Fixture(*this);
		UPGXInputSubsystem* Input = Fixture.Get();
		if (!TestNotNull(*FString::Printf(TEXT("SettingsConfigLifecycle cycle %d subsystem"), Cycle + 1), Input))
		{
			continue;
		}
		TestTrue(*FString::Printf(TEXT("SettingsConfigLifecycle cycle %d uses configured asset"), Cycle + 1),
			Input->GetActiveInputConfig() == Config);
		TestEqual(*FString::Printf(TEXT("SettingsConfigLifecycle cycle %d buffer capacity"), Cycle + 1),
			Input->GetInputBuffer()->GetCapacity(), 7);
		TestTrue(*FString::Printf(TEXT("SettingsConfigLifecycle cycle %d buffer window"), Cycle + 1),
			FMath::IsNearlyEqual(Input->GetInputBuffer()->GetWindowSeconds(), 0.35));
		TestEqual(*FString::Printf(TEXT("SettingsConfigLifecycle cycle %d context count"), Cycle + 1),
			Input->GetCount(), 1);
		TestTrue(*FString::Printf(TEXT("SettingsConfigLifecycle cycle %d default context"), Cycle + 1),
			Input->FindContextAsset(Context->ContextTag) == Context);
	}

	Settings->ActiveConfig = TSoftObjectPtr<UPGXInputConfig>(FSoftObjectPath(TEXT("/PGXInput/Tests/MissingConfig.MissingConfig")));
	{
		PGXInputAutomation::FScopedInputSubsystemFixture Fixture(*this);
		UPGXInputSubsystem* Input = Fixture.Get();
		if (TestNotNull(TEXT("SettingsConfigLifecycle invalid reference fallback subsystem"), Input))
		{
			TestNotNull(TEXT("SettingsConfigLifecycle invalid reference creates safe config"), Input->GetActiveInputConfig());
			TestEqual(TEXT("SettingsConfigLifecycle invalid reference has no contexts"), Input->GetCount(), 0);
			TestEqual(TEXT("SettingsConfigLifecycle invalid reference uses default buffer capacity"),
				Input->GetInputBuffer()->GetCapacity(), 16);
		}
	}

	Settings->ActiveConfig.Reset();
	{
		PGXInputAutomation::FScopedInputSubsystemFixture Fixture(*this);
		UPGXInputSubsystem* Input = Fixture.Get();
		if (TestNotNull(TEXT("SettingsConfigLifecycle unset reference fallback subsystem"), Input))
		{
			TestNotNull(TEXT("SettingsConfigLifecycle unset reference creates safe config"), Input->GetActiveInputConfig());
			TestEqual(TEXT("SettingsConfigLifecycle unset reference has no contexts"), Input->GetCount(), 0);
		}
	}

	Settings->ActiveConfig = OriginalConfig;
	return true;
}
