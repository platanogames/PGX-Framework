...
namespace
{
	const FGameplayTagContainer& GetAbilityAssetTags(const UGameplayAbility* Ability)
	{
		static const FGameplayTagContainer EmptyTags;
		return Ability ? Ability->GetAssetTags() : EmptyTags;
	}

	FGameplayTag GetAbilityIdentityTag(const UGameplayAbility* Ability)
	{
		const FGameplayTagContainer& AssetTags = GetAbilityAssetTags(Ability);
		return AssetTags.Num() > 0 ? AssetTags.First() : FGameplayTag();
	}
}
...
FPGXAbilityResult UPGXAbilityFacade::GrantAbility(TSubclassOf<UGameplayAbility> AbilityClass, int32 Level, FPGXAbilityHandle& OutHandle)
{
	...
	const FGameplayTag IdentityTag = GetAbilityIdentityTag(AbilityCDO);
	...
	if (IdentityTag.IsValid())
	{
		...
		{
			if (GetAbilityAssetTags(ExistingSpec.Ability).HasTag(IdentityTag))
			{
...
bool UPGXAbilityFacade::IsAbilityGranted(FGameplayTag AbilityTag) const
{
	...
	{
		if (GetAbilityAssetTags(Spec.Ability).HasTag(AbilityTag))
		{
			return true;
		}
	}
...
TArray<FPGXAbilitySnapshot> UPGXAbilityFacade::GetGrantedAbilities() const
{
	...
	for (const FGameplayAbilitySpec& Spec : ASC->GetActivatableAbilities())
	{
		...
		Snapshot.Handle.AbilityTag = GetAbilityIdentityTag(Spec.Ability);
