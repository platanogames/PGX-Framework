...
void UPGXAbilitySubsystem::ExecuteConsoleCommand(const FString& CommandName, const TArray<FString>& Args, UWorld* World)
{
	if (CommandName == TEXT("pgx.ability.components"))
	{
		if (!World) return;
	...
	if (CommandName == TEXT("pgx.ability.status"))
	{
		if (!World) return;
		UGameInstance* GameInstance = World->GetGameInstance();
		UPGXAbilitySubsystem* Sub = GameInstance ? GameInstance->GetSubsystem<UPGXAbilitySubsystem>() : nullptr;
