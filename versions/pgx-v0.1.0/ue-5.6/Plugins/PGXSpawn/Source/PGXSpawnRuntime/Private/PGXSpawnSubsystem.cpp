...
void UPGXSpawnSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	// console diagnostics: Register console commands (pgx.spawn.*)
	IConsoleManager& Manager = IConsoleManager::Get();
	IConsoleCommand* Cmd = nullptr;

	Cmd = Manager.RegisterConsoleCommand(
		TEXT("pgx.spawn.list"),
		TEXT("PGXSpawn: dump all active spawn records to log."),
		FConsoleCommandWithArgsDelegate::CreateUObject(this, &UPGXSpawnSubsystem::HandleConsoleList),
		ECVF_Default);
	if (Cmd) RegisteredConsoleCommands.Add(Cmd);

	Cmd = Manager.RegisterConsoleCommand(
		TEXT("pgx.spawn.cleanup"),
		TEXT("PGXSpawn: force cleanup of all inactive records."),
		FConsoleCommandWithArgsDelegate::CreateUObject(this, &UPGXSpawnSubsystem::HandleConsoleCleanup),
		ECVF_Default);
	if (Cmd) RegisteredConsoleCommands.Add(Cmd);
	...
	Cmd = Manager.RegisterConsoleCommand(
		...
		FConsoleCommandWithArgsDelegate::CreateUObject(this, &UPGXSpawnSubsystem::HandleConsoleBudget),
		ECVF_Default);
	if (Cmd) RegisteredConsoleCommands.Add(Cmd);
	...
	Cmd = Manager.RegisterConsoleCommand(
		...
		FConsoleCommandWithArgsDelegate::CreateUObject(this, &UPGXSpawnSubsystem::HandleConsoleWaves),
		ECVF_Default);
	if (Cmd) RegisteredConsoleCommands.Add(Cmd);
	...
	Cmd = Manager.RegisterConsoleCommand(
		...
		FConsoleCommandWithArgsDelegate::CreateUObject(this, &UPGXSpawnSubsystem::HandleConsoleTriggerPoint),
		ECVF_Default);
	if (Cmd) RegisteredConsoleCommands.Add(Cmd);
	...
	Cmd = Manager.RegisterConsoleCommand(
		...
		FConsoleCommandWithArgsDelegate::CreateUObject(this, &UPGXSpawnSubsystem::HandleConsolePoolClear),
		ECVF_Default);
	if (Cmd) RegisteredConsoleCommands.Add(Cmd);
}
...
{
	// console diagnostics: unregister console commands
	IConsoleManager& Manager = IConsoleManager::Get();
	for (IConsoleCommand* Cmd : RegisteredConsoleCommands)
	{
		if (Cmd)
		{
