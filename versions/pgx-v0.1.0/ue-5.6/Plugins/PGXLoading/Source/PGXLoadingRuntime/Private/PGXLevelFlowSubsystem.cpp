...
void UPGXLevelFlowSubsystem::ExecuteOpenLevel()
{
	...
	PGX_LOG_INFO(LogPGXLoading, TEXT("[LevelFlow] OpenLevel: %s"), *MapPath);
...
void UPGXLevelFlowSubsystem::RegisterConsoleCommands()
{
	IConsoleManager& CM = IConsoleManager::Get();

	// --- pgx.level.status ---
	if (IConsoleObject* Cmd = CM.RegisterConsoleCommand(
		TEXT("pgx.level.status"),
		TEXT("Show LevelFlow status: current state, level, sub-levels"),
	...
	if (IConsoleObject* Cmd = CM.RegisterConsoleCommand(
		TEXT("pgx.level.load"),
		TEXT("Request level transition: pgx.level.load <GameplayTag>"),
	...
	// --- pgx.level.cancel ---
	if (IConsoleObject* Cmd = CM.RegisterConsoleCommand(
		TEXT("pgx.level.cancel"),
		TEXT("Cancel active level transition"),
		FConsoleCommandDelegate::CreateWeakLambda(this, [this]()
		{
	...
	// --- pgx.level.profiles ---
	if (IConsoleObject* Cmd = CM.RegisterConsoleCommand(
		TEXT("pgx.level.profiles"),
		TEXT("List discovered level profiles"),
		FConsoleCommandDelegate::CreateWeakLambda(this, [this]()
		{
	...
	// --- pgx.level.resolve <Tag> ---
	if (IConsoleObject* Cmd = CM.RegisterConsoleCommand(
		TEXT("pgx.level.resolve"),
		TEXT("Resolve tag and show level data: pgx.level.resolve <Tag>"),
		FConsoleCommandWithArgsDelegate::CreateWeakLambda(this, [this](const TArray<FString>& Args)
		{
			if (Args.Num() < 1)
			{
	...
	// --- pgx.level.history ---
	if (IConsoleObject* Cmd = CM.RegisterConsoleCommand(
		TEXT("pgx.level.history"),
		TEXT("Show transition history with timing data"),
		FConsoleCommandDelegate::CreateWeakLambda(this, [this]()
		{
	...
	// --- pgx.level.sublevels ---
	if (IConsoleObject* Cmd = CM.RegisterConsoleCommand(
		TEXT("pgx.level.sublevels"),
		TEXT("List sub-levels and their load state"),
		FConsoleCommandDelegate::CreateWeakLambda(this, [this]()
		{
	...
	// --- pgx.level.entrypoints ---
	if (IConsoleObject* Cmd = CM.RegisterConsoleCommand(
		TEXT("pgx.level.entrypoints"),
		TEXT("Show entry points from current LevelFlowActor"),
		FConsoleCommandDelegate::CreateWeakLambda(this, [this]()
		{
...
void UPGXLevelFlowSubsystem::UnregisterConsoleCommands()
{
	IConsoleManager& CM = IConsoleManager::Get();
